<div class="simplecheckout-block" id="simplecheckout_shipping" <?php echo $hide ? 'data-hide="true"' : '' ?> <?php echo $display_error && $has_error ? 'data-error="true"' : '' ?>>
    <?php if ($display_header) { ?>
        <div class="checkout-heading panel-heading" style="display: inline-block;"><?php echo $text_checkout_shipping_method ?></div>
    <?php } ?>
    ...
    <div class="simplecheckout-block-content" style="display: inline-block;">
                    ...
                    <?php foreach ($shipping_methods as $shipping_method) { ?>
                        ...
                        <?php if (empty($shipping_method['error'])) { ?>
                            <?php foreach ($shipping_method['quote'] as $quote) { ?>
                                <option value="<?php echo $quote['code']; ?>" <?php echo !empty($quote['dummy']) ? 'disabled="disabled"' : '' ?> <?php echo !empty($quote['dummy']) ? 'data-dummy="true"' : '' ?> <?php if ($quote['code'] == $code) { ?>selected="selected"<?php } ?>><?php echo $quote['title']; ?><!--<?php echo !empty($quote['text']) ? ' - '.$quote['text'] : ''; ?> цена 0,00грнв выборе доставки--></option>
